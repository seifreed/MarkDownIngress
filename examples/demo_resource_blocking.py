...
async def demo_resource_blocking():
    ...
    if result_no_block and result_with_block:
        ...
        time_pct = (time_diff / result_no_block.timing_ms * 100) if result_no_block.timing_ms > 0 else 0
        ...
        if 'blocked_requests' in result_with_block.metadata:
            ...
            print(f"Blocked {blocked} out of {total} requests ({result_with_block.metadata['block_rate_pct']:.1f}%)")
