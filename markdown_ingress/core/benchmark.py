...
class Benchmark:
    ...
    def run_single(self, url: str, mode: str = "fast", iterations: int = 3) -> BenchmarkResult:
        """
    ...
    def run_batch(
        self, urls: list[str], mode: str = "fast", iterations: int = 3
    ...
    def generate_report(self, results: list[BenchmarkResult]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            report_lines.append(
                f"   Tokens: {result.original_tokens:,} → {result.cleaned_tokens:,} (-{result.reduction_percent:.1f}%)"
            )
            report_lines.append(
                f"   Size: {result.original_size_bytes:,} → {result.cleaned_size_bytes:,} bytes (-{result.size_reduction_percent:.1f}%)"
            )
