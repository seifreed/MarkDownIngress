...
class Renderer:
    ...
    async def _render_with_progressive_timeout(self, url: str) -> FetchResult:
        ...
        for strategy_index, (wait_state, timeout_ms) in enumerate(self.LOAD_STRATEGIES):
            try:
                logger.info(f"[Extreme Mode] Attempt {strategy_index + 1}/3: {wait_state} ({timeout_ms/1000}s)")
        ...
        logger.error(f"[Extreme Mode] All progressive timeout strategies failed")
    ...
    async def _wait_for_content(self, page, max_wait: int = 30):
        ...
        try:
            ...
            logger.info(f"[Smart Wait] Content verification passed")
        except Exception as e:
