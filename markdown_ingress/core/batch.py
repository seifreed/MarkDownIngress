...
class BatchProcessor:
    ...
    async def process_url(self, url: str) -> SafeDocument:
        ...
        if self.mode == "auto":
            ...
            doc = await loop.run_in_executor(
                None,
                lambda: ingest(
                    url, mode="auto", strict=self.strict, model=self.model, timeout=self.timeout
                ),
            )
            return doc
    ...
    async def process_batch_async(self, urls: list[str]) -> BatchResult:
        ...
        return BatchResult(
            total=total, successful=successful, failed=failed, documents=documents, errors=errors
        )
