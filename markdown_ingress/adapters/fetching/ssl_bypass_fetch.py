...
def _raise_ssl_bypass_exhausted(url: str, last_exc: Exception | None) -> NoReturn:
    if last_exc is not None:
        raise last_exc
    raise RuntimeError(f"SSL bypass fetch failed for {url} after {MAX_RETRIES} attempts")


class SslBypassFetchMixin:
    ...
    async def _fetch_with_ssl_bypass(
        ...
        *,
        ...
        if remaining_attempts == 0:
            _raise_ssl_bypass_exhausted(state.url, last_exc)
        ...
        _raise_ssl_bypass_exhausted(state.url, ssl_last_exc)
